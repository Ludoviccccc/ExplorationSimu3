...
class OptimizationPolicykNN(test_programs):
    ...
    def __call__(self,goal:np.ndarray,H:History,stats:dict,cond:np.ndarray=None)->dict:
        closest_codes = self.select_closest_codes(H,goal,stats,cond) #most promising sample from the history
    ...
    def loss(self,goal:np.ndarray, elements:dict):
        ...
        #out = np.sum((a - (elements['features']- min_)/denominator)**2,axis=1)
    ...
    def feature2closest_code(self,features:dict,signature:np.ndarray)->np.ndarray:
        ...
        return idx
    def select_closest_codes(self,H:History,signature: dict,stats:dict,cond:np.ndarray=None)->dict:
